...
def batch_iterator_chat_completions(
    ...
    while True:
        ...
        inputs = tokenize_instructions_fn(
            instructions=instructions_batch, outputs=outputs_batch
        )
        ...
        for b in range(inputs["input_ids"].shape[0]):
            for i in range(inputs["input_ids"].shape[1]):
                if torch.all(
                    inputs["input_ids"][b, i : i + eoi_toks.shape[0]] == eoi_toks
                ):
                ...
                if (
                    eoi_toks.shape[0] == 6
                    and (inputs["input_ids"][b, i : i + eoi_toks.shape[0]] == eoi_toks)
                    .sum()
                    .item()
                    >= eoi_toks.shape[0] - 2
                ):
...
def batch_iterator_custom_completions(
    completions_file_path: str, tokenize_instructions_fn, batch_size, eoi_toks
):
    ...
    return batch_iterator_chat_completions(
        instructions, completions, tokenize_instructions_fn, batch_size, eoi_toks
    )
...
def compute_loss_over_dataset(
    model, tokenizer, batch_iterator, n_batches=256, fwd_pre_hooks=[], fwd_hooks=[]
):
    ...
    for inputs, loss_mask in batch_iterator:
        ...
        log_probs_for_labels = torch.cat(
            [
                log_probs_for_labels,
                torch.zeros(log_probs_for_labels.shape[0])
                .unsqueeze(-1)
                .to(log_probs_for_labels),
            ],
            dim=-1,
        )

        # apply loss_mask
        log_probs_for_labels = log_probs_for_labels * loss_mask.to(
            log_probs_for_labels.device
        )
...
def evaluate_loss(
    ...
    for label in dataset_labels:
        if label == "pile":
            dataset_iterator = batch_iterator_pile(
                model_base.tokenizer, batch_size=batch_size, max_length=max_seq_length
            )
            n = n_batches
        elif label == "alpaca":
            dataset_iterator = batch_iterator_alpaca(
            ...
            assert completions_file_path is not None, (
                "A file path must be passed to load the completions"
            )
        ...
        print(
            f"CE loss: {ce_loss.item()}, Perplexity: {perplexity.item()}, N tokens: {n_tokens.item()}"
        )
